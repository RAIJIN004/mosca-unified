"""Mosca LATERAL: copia intacta de auto_bot + UNA sola adicion (filtro deriva leve).
LATERAL = lateralizada ligeramente a favor: LONG exige deriva24h en [-3,+15]%;
SHORT en [-15%,+3%]. Fuera parabolicos y contratendencias fuertes (movimientos
limpios sin retrocesos bruscos). Todo lo demas IDENTICO al original.
Tag moscaL-, estado/log propios.
"""
...
def run_once(ctx, risk_usd, short_half=True):
    ...
    if st["pnl_R"] > -3.0 and npos < MAXPOS:
        ...
        try:
            ...
            drift = {t["symbol"]: float(t.get("priceChangePercent", 0) or 0) for t in tk}
        except Exception:
            drift = {}
        for sym in shard_syms():
            ...
            try:
                ...
                for i in [len(ks) - 3, len(ks) - 2]:
                    ...
                    # ADICION lateral: deriva 24h leve a favor (lateral, sin parabolicos ni contratendencias)
                    dr = drift.get(sym, 0)
                    if side == 1 and not (-3 <= dr <= 15): continue
                    if side == -1 and not (-15 <= dr <= 3): continue
...
def selftest():
    ...
    ok("kill -3R", (-3.0 <= -3.0))
    ok("prefijo tag lateral", TAG == "moscaL-")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--tag", default="moscaL-", help="prefijo de ordenes propio de esta mosca.")
    ap.add_argument("--state", default="auto_stateL.json", help="archivo de estado propio.")
    ap.add_argument("--log", default="tradesL.csv", help="log propio.")
