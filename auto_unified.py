"""Mosca UNIFIED: universo dinamico estilo trend-unified + entrada campeon V3-deep.
Seleccion: top volumen 24h (excluye basura/estables/no-crypto/majors) rankeado por escalera
de volumen 1h (v6>v12>v24). Deriva leve a favor (LONG: 24h>=-5%; SHORT: <=+5%).
Book alineado opcional (--book, sin backtest: default OFF).
Entrada/gestion: IDENTICAS al campeon (frac 0.8, TP 1R, filtros eff/alpha/noPBvol).
Tag: moscaU- (no pisa a la original).
"""
...
MAXPOS = 5
ADOPT = False
BOOK = False  # --book lo activa (sin backtest)
JUNK = ("UPUSDT", "DOWNUSDT", "BULL", "BEAR")
STABLES = ("USDC", "FDUSD", "TUSD", "DAI", "USDP", "AEUR", "EUR")
NONCRYPTO = ("AAPL", "TSLA", "NVDA", "META", "AMD", "INTC", "QQQ", "SPY", "TQQQ", "SQQQ",
             "MSFT", "AMZN", "GOOGL", "NFLX", "COIN", "MSTR", "XAU", "XAUT", "PAXG")
MAJORS = ("BTC", "ETH", "SOL", "XRP", "BNB", "DOGE", "ADA", "TRX", "LINK", "XLM",
          "SUI", "AVAX", "SHIB", "HBAR", "DOT", "LTC", "NEAR", "APT", "ARB", "ATOM", "OP")
def shard_syms():
    i, n = (int(x) for x in SHARD.split("/"))
    return SYMS[i::n]
def shard_syms_universe(uni):
    i, n = (int(x) for x in SHARD.split("/"))
    return uni[i::n]
def unified_universe(ctx, top_vol=40, top_n=15):
    """Top volumen menos basura, rankeado por escalera 1h. Retorna [(sym, v6, drift24)]."""
    info = ctx.req("GET", "/fapi/v1/exchangeInfo")
    live = {s["symbol"] for s in info["symbols"] if s.get("status") == "TRADING"}
    tk = ctx.req("GET", "/fapi/v1/ticker/24hr")
    cands = []
    for t in tk:
        s = t["symbol"]
        if s not in live: continue
        if not s.endswith("USDT") or any(x in s for x in JUNK): continue
        if s[:-4] in STABLES or s[:-4] in NONCRYPTO or s[:-4] in MAJORS: continue
        if float(t.get("quoteVolume", 0) or 0) < 5_000_000: continue
        cands.append((s, float(t["quoteVolume"]), float(t.get("priceChangePercent", 0) or 0)))
    cands.sort(key=lambda x: -x[1])
    cands = cands[:top_vol]
    out = []
    for s, _, drift in cands:
        try:
            kh = ctx.req("GET", "/fapi/v1/klines", {"symbol": s, "interval": "1h", "limit": 260})
            hv = [float(k[5]) * float(k[4]) for k in kh]
            if len(hv) < 260: continue
            def mult(h):
                w = hv[-h:]; b = hv[-h-200:-h]
                return (sum(w) / len(w)) / (sum(b) / len(b)) if sum(b) > 0 else 0
            v24, v12, v6 = mult(24), mult(12), mult(6)
            out.append((s, v6, v12, v24, drift))
        except Exception:
            continue
    out.sort(key=lambda x: (-(x[1] > x[2] > x[3]), -x[1]))
    return [(s, v6, drift) for s, v6, _, _, drift in out[:top_n]]
def book_bias(ctx, sym):
    try:
        ob = ctx.req("GET", "/fapi/v1/depth", {"symbol": sym, "limit": 20})
        bids = [(float(p), float(q)) for p, q in ob.get("bids", [])]
        asks = [(float(p), float(q)) for p, q in ob.get("asks", [])]
        bv = sum(p * q for p, q in bids); av = sum(p * q for p, q in asks)
...
def run_once(ctx, risk_usd, short_half=True):
    ...
    for sym, p in poss.items():
        ...
        if abs(pa) < 1e-9 or sym in st["managed"] or sym not in SYMS:
            continue  # alcance: solo universo base; resto lo audita el watcher
        if ADOPT:
    ...
    # 4. scan UNIFIED: universo dinamico (top vol rankeado por escalera) + entrada campeon
    ...
    if st["pnl_R"] > -3.0 and npos < MAXPOS:
        ...
        uni = unified_universe(ctx)
        acts.append(("universo", f"{len(uni)} coins", ",".join(s for s, _, _ in uni[:5]) + "..."))
        dmap = {s: d for s, _, d in uni}
        for sym in shard_syms_universe([s for s, _, _ in uni]):
            if npos >= MAXPOS: break
            try:
                drift = dmap.get(sym, 0)
                ks = ctx.kl(sym)
                ...
                for i in [len(ks) - 3, len(ks) - 2]:
                    ...
                    # deriva leve a favor (lateral unified): LONG exige 24h>=-5%, SHORT <=+5%
                    if side == 1 and drift < -5: continue
                    if side == -1 and drift > 5: continue
                    # book alineado opcional (--book; sin backtest)
                    if BOOK:
                        imb = book_bias(ctx, sym)
                        if side == 1 and imb < 0.05: continue
                        if side == -1 and imb > -0.05: continue
...
def selftest():
    ...
    ok("kill -3R", (-3.0 <= -3.0))
    ok("prefijo tag unified", TAG == "moscaU-")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--tag", default="moscaU-", help="prefijo de ordenes propio de esta mosca.")
    ap.add_argument("--state", default="auto_stateU.json", help="archivo de estado propio.")
    ap.add_argument("--log", default="tradesU.csv", help="log propio.")
    ap.add_argument("--max-pos", type=int, default=5, help="tope de posiciones de esta mosca.")
    ap.add_argument("--book", action="store_true", help="exige book alineado (unified TOP; SIN backtest).")
    ...
    BOOK = a.book  # scope modulo: asignacion directa (sin global: ya es scope de modulo)
    if a.selftest:
